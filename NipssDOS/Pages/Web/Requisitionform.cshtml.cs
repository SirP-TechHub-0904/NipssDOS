...
namespace NipssDOS.Pages.Web
{
    public class RequisitionformModel : PageModel
    {
        ...
        public RequisitionformModel(NipssDOS.Data.NIPSSDbContext context, IHostingEnvironment hostingEnv, UserManager<IdentityUser> userManager)
        {
            _context = context;
        ...
        int imgCount = 0;
                ...
                string mi = $"Your Request has been booked with Ticket Number ("+Ticket.TicketNumber+") <br>Priority: " + Ticket.Priority + "<br>" +
                    ...
                    $"House/Office Number: " + Ticket.HouseOfficeNumber + "<br><strong>Details</strong><br>" + Ticket.Details;


                string mailmsg = sr.ReadToEnd();
                mailmsg = mailmsg.Replace("{NAME}", Ticket.Fullname);
                mailmsg = mailmsg.Replace("{TITLE}", "Work and Maintenance Department");
                ...
                try
                {
                    ...
                    foreach (var mm in ticketsupervisor)
                    {
                        ...
                        if (mm.SendEmail == true)
                        {
                            ...
                            mms.Mail = "New Request has been booked with Ticket Number<br>"+ mi;
                ...
                return RedirectToPage("/TicketPage/Index", new { ticketnumber = Ticket.TicketNumber, area="Participant" });
            }
            catch (Exception c)
            {
                ViewData["Category"] = new SelectList(_context.TicketCategories, "Title", "Title");
        ...
        public async Task<bool> SendEmail(string recipient, string message, string title)
        {
            try
            {
                ...
                mail.Body = message;
                //set the addresses 
                mail.From = new MailAddress("noreply@NipssDOS.com", "SEC44 NIPSS"); //IMPORTANT: This must be same as your smtp authentication address.
                ...
                //send the message 
                SmtpClient smtp = new SmtpClient("mail.NipssDOS.com");

                //IMPORANT:  Your smtp login email MUST be same as your FROM address. 
                NetworkCredential Credentials = new NetworkCredential("noreply@NipssDOS.com", "Admin@123");
