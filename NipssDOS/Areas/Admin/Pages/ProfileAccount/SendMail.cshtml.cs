...
namespace NipssDOS.Areas.Admin.Pages.ProfileAccount
{
    public class SendMailModel : PageModel
    {
        ...
        public SendMailModel(NipssDOS.Data.NIPSSDbContext context, IHostingEnvironment hostingEnv, SignInManager<IdentityUser> signInManager, UserManager<IdentityUser> userManager)
        {
            _context = context;
        ...
        public async Task<IActionResult> OnGetAsync(int? id)
        {
            ...
            string mi = $"Kindly Login via the link <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>click here</a>.<br>" +
                $"<h5>OR Visit <a href='www.NipssDOS.com'>www.NipssDOS.com</a> and click on VLE on the menu.</h6><br>"+
        ...
        public async Task<bool> SendEmail(string recipient, string message, string title)
        {
            try
            {
                ...
                mail.Body = message;
                //set the addresses 
                mail.From = new MailAddress("no-reply@NipssDOS.com", "SEC44 NIPSS"); //IMPORTANT: This must be same as your smtp authentication address.
                ...
                NetworkCredential Credentials = new NetworkCredential("no-reply@NipssDOS.com", "Admin@123");
